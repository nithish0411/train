...
void binary_tree:: show(node* n)
{
    ...
    cout << "Name: " << n->user_node->get_name() << ", Age: " << n->user_node->get_age()
         << ", Gender: " << n->user_node->get_gender()
         << ", Preference: " << n->user_node->get_preference() << endl;
    show(n->left);
    cout<<"right"<<endl;
    show(n->right);
}
...
void functions(node* l)
{
	if(l)
	{
		while(1)
		{
			cout<<"1=> select train 2=> show bookings 3=>delete account 4=>logout"<<endl;
			int ch;
			cin>>ch;
			switch(ch)
			{
